...
def find_shortest_path(weights, x0=None, xf=None):
    ...
    for y in range(1, h):
        ...
        for x in range(w):
            ...
            new_weight = S[y-1, max(x-1,0)] + weights[y,x] + sqrt2
        ...
        for x in range(w-1, -1, -1):
            weight = weight + weights[y,x] + 1
            new_weight = S[y-1, min(x+1, w-1)] + weights[y,x] + sqrt2
...
def plot_path(path, **kwargs):
    ...
    xx = np.zeros(2 * n - 1)
    xx[0::2] = path
    xx[1::2] = path[:-1] + np.sign(path[1:] - path[:-1])
    yy = np.zeros(2 * n - 1)
